...
if __name__ == '__main__':
    ...
    for i in range(len(eps)):
        ...
        t, T = gas.DE(pmax = 5, NSIDE = 8)
        for j in range(1,4):
            
            ax0.plot(t*gas.wadv,T[:, j*150, 2], color = colors[i], ls = linestyles[j])
    ...
    #gas.print_stuff()
    t, T = gas.DE(pmax = 15, steps = 300, NSIDE = 8)
    for i in range(8):
        ...
        #print pos
        temp, = ax2.plot(t,T[:,328+3*i,2])
    t,r = gas.radius(pmax = 15)
    
    #phi2 = self.wadv*t
    #phi = phi2/np.pi
    
    radius, = ax3.plot(t,r/gas.AU, linestyle = '--', color = 'k')
    #ax2.set_xlim(2,8)
...
for i in range(len(eps)-1):
    for j in range(2):
        gas = p(e =0, wadv = 1.1, epsilon=eps[i], Porb = 3.0)
        #gas.print_stuff()
        t, d = gas.DE(pmax = 3.0)
        ax0.plot(d[:,338 - 30*j,1],d[:,338-30*j,2], color = colors[i], ls = linestyles[j])
        #"line"+str(i)= ax0.plot(phi,T, color = colors[i], ls = linestyles[j])
        #labels.append("line" + str[i])
        #names.append("eps = "+str(eps[i])+", \Theta = "+ str(thetas[j]) )
...
ax0.set_xticklabels(['$0$', '$\pi/2$', '$\pi$','$3\pi/2$', '$2\pi/$'], fontsize  = 14)
ax0.set_xlabel(" $\phi$ ", fontsize = 14)
ax0.set_ylabel("$T/T0*sin\Theta^{1/4}$",fontsize = 14)
ax0.set_title('Temperature of gas parcel from dawn till dawn - circular orbit')
...
for i in range(len(eps1)):
        ...
        gas = p(epsilon=eps1[i], wadv = 1.1, Porb = 3.0)
        
        Tmaxi, Tdawni, Tduski  = gas.findT(pmax = 3, steps =300)
        Tm.append(Tmaxi)
        Tda.append(Tdawni[0])
        Tdu.append(Tduski[0])
        
        
maxim, = ax1.semilogx(eps1,gas.Tmax(eps1), color = 'r')
dawn, = ax1.semilogx(eps1,gas.Tdawn(eps1),color = 'b')
dusk, = ax1.semilogx(eps1,gas.Tdusk(eps1),color = 'k')
...
ee = 0.3
gas = p(e = ee, epsilon=3, wadv = 1.1, Porb = 3.2)
#gas.print_stuff()
for i in range(8):
    t, d = gas.DE(pmax = 10, steps = 300)
    #phi= np.linspace(0,2*30.0, num=300*30)
    #temp, = ax2.plot((phi*np.pi)/(gas.wadv*gas.P),T)
    temp, = ax2.plot(t,d[:,368-30*i,2])

t,r = gas.radius(pmax = 10)
...
ee = 0.5
gas = p(e = ee, epsilon=3, wadv = 1.1, Porb = 2.5)
#gas.print_stuff()
for i in range(5):
    t, d = gas.DE(pmax = 10, steps = 300)
    #phi= np.linspace(0,2*30.0, num=300*30)
